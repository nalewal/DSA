...
public:
    int getIntegerFromString(string str) {
       ...
       int num = 0;
       while (end >= start) {
           int integer = str[end--] - '0';
           num = num + (integer * addten);
           addten = addten * 10;
       }
       return num;
    }
    bool areNumbersAscending(string s) {
        ...
        for (int i = 0; i < s.length(); i++) {
            int range = s[i] - '0'; // 0 to 9
            cout << " character is => " << s[i] << endl;
            cout << " temp is => " << temp << endl;
            if (s[i] == ' ' && temp != "") {
                int number = getIntegerFromString(temp);
                cout << " Number is => " << number << endl;
                cout << " Prev number is => " << previousNumber << endl;
        ...
        if (temp != "") {
            int lastNumber = getIntegerFromString(temp);
